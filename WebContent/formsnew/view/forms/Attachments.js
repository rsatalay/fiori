...
sap.umc.mobile.forms.view.forms.Attachments={
   ...
   /* handleUploadBtn:function(uploadEnable){
        var oUploadCollection = this.getView().getController()._AttachmentsFragment;
      oUploadCollection.setUploadEnabled(uploadEnable);

    },*/
   // beforeUploadStarts:function(){alert("goitju");},
    onUploadTerminated:function(evt){
    ...
    onChange: function(oEvent) {
      if(!oThis.oCurrentFormBundle.FormBundleID){
		/*
			  ...
			  */
       oThis.getDataProvider().processForm(oThis,sap.umc.mobile.private.app.Constants.FORM_STATUS.DRAFT, true,true);
       ...
       if(!oThis.oCurrentFormBundle.FormBundleID){
        ...
        parent.getFilterByKey("attachments").removeAllContent();
        parent.oIconTabBar.setSelectedKey("attachments");
        parent.oIconTabBar.fireSelect();
        ...
        var sSlug = "Description="+oEvent.getParameters().getParameter("files")[0].name+";FileName='"+oEvent.getParameters().getParameter("files")[0].name+"';DocumentCategory='GENI';FormBundleID='"+parent.oCurrentFormBundle.FormBundleID+"'";
    ...
    _loadAttachments: function(){
        ...
        var fnCallback = jQuery.proxy(function(oAttachments) {
          ...
          if(oAttachments.results.length == 0){
            ...
            this.oAttachments.setData(oAttachments);
            this.getView().setModel(this.oAttachments, "FileAttachments");}
           /* if(((this.getView().getController().oCurrentFormBundle.StatusID)?this.getView().getController().oCurrentFormBundle.StatusID.toLowerCase():this.getView().getController().oCurrentFormBundle.StatusID) === sap.umc.mobile.private.app.Constants.FORM_STATUS.SUBMIT.toLowerCase()){
